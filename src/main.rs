use anyhow::Result;
use llminate::{cli::Cli, config::LoggingConfig, error};
...
async fn init_tracing(config: LoggingConfig, is_print_mode: bool) -> Result<()> {
    ...
    let mut filter_string = format!(
        "llminate={},tokio=info,hyper=info,reqwest=info",
        default_level
    );
    ...
    let env_filter =
        EnvFilter::try_from_default_env().or_else(|_| EnvFilter::try_new(&filter_string))?;
    ...
    if enable_stdout && enable_file && enable_json {
        // All three layers - console (stderr) + file + json
        ...
        // Print mode: stdout is reserved for the JSON-lines protocol.
        // No tracing output unless explicitly enabled via --stderr-logs or --debug.
        // Install subscriber with only the filter — events are silently discarded.
        registry.init();
    } else {
        // Fallback to basic stdout logging (non-print mode only)
        registry
    ...
    tracing::debug!(
        "Config: default_level={:?}, modules={:?}, stdout={}, file={}, json={}",
