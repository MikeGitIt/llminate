    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        if self.is_processing && self.task_status.is_some() {
            ...
            all_lines.push(Line::from(vec![
                ...
                Span::raw(" "),
                Span::styled(
                    "(esc to interrupt • ctrl+r to expand)",
                    Style::default()
                        .fg(Color::Gray)
                        .add_modifier(Modifier::ITALIC),
                ),
        ...
        let highlighted_lines = if let (Some(start), Some(end)) =
            ...
            all_lines
                ...
                .map(|(line_idx, line)| {
                    if line_idx < start.0 || line_idx > end.0 {
                        ...
                        let line_text: String = owned_line
                            .spans
                            .iter()
...
impl<'a> Widget for StatusBar<'a> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        let chunks = Layout::default()
            ...
            .constraints([
                Constraint::Length(22), // Mode (wider for [PLAN] prefix)
        ...
        // Mode indicator (with plan mode)
        let mode = if self.state.plan_mode {
            if self.state.is_processing {
                Span::styled("[PLAN] PROCESSING", Style::default().fg(Color::Magenta))
            } else if self.state.input_mode {
                Span::styled("[PLAN] INPUT", Style::default().fg(Color::Magenta))
            } else {
                Span::styled("[PLAN] READY", Style::default().fg(Color::Magenta))
            }
        } else if self.state.input_mode {
