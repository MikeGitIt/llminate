...
use crate::ai::{ChatRequest, ContentPart, Message, MessageContent, MessageRole, Tool};
...
static FETCH_CACHE: Lazy<Arc<Mutex<LruCache>>> =
    Lazy::new(|| Arc::new(Mutex::new(LruCache::new(100))));
...
async fn fetch_url(url: &str) -> Result<CachedFetchResult> {
    ...
    let parsed_url =
        Url::parse(url).map_err(|e| Error::InvalidInput(format!("Invalid URL: {}", e)))?;
                ...
                if let Some(location) = resp.headers().get("location") {
                    ...
                    if original_host != redirect_host {
                        return Err(Error::Network(format!(
                            "Redirect to different host detected. Please fetch from: {}",
                            redirect_url
                        )));
                    }
    ...
    let final_content = if processed_content.len() > MAX_PROCESSED_LENGTH {
        format!(
            "{}...[content truncated]",
            &processed_content[..MAX_PROCESSED_LENGTH]
        )
    } else {
        processed_content
    };
...
impl ToolHandler for WebFetchTool {
    ...
    async fn execute(
        ...
        let ai_result = process_content_with_ai(&fetch_result.content, prompt)
            .await
            .unwrap_or_else(|e| {
        ...
        Ok(format!(
            "{}\n\n[HTTP Status: {} {}]",
            ai_result, fetch_result.code, fetch_result.code_text
        ))
    }
    ...
    async fn execute(
        ...
        if query.len() < 2 {
            return Err(Error::InvalidInput(
                "Query must be at least 2 characters long".to_string(),
            ));
        }

        let allowed_domains = input["allowed_domains"].as_array().map(|arr| {
            arr.iter()
        ...
        let blocked_domains = input["blocked_domains"].as_array().map(|arr| {
            arr.iter()
                .filter_map(|v| v.as_str())
                .map(|s| s.to_string())
                .collect::<Vec<_>>()
