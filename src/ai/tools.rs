...
use crate::ai::emacs_tool::EmacsCommandTool;
use crate::ai::enter_plan_mode_tool::EnterPlanModeTool;
use crate::ai::exit_plan_mode_tool::ExitPlanModeTool;
use crate::ai::notebook_tools::{NotebookEditTool, NotebookReadTool};
use crate::ai::skill_tool::SkillTool;
use crate::ai::task_tools::{TaskCreateTool, TaskGetTool, TaskListTool, TaskUpdateTool};
use crate::ai::todo_tool::{TodoReadTool, TodoWriteTool};
use crate::ai::web_tools::{WebFetchTool, WebSearchTool};
use crate::ai::{ContentPart, Tool};
use crate::error::{Error, Result};
use crate::hooks::{execute_hooks, HookContext, HookType};
use crate::tui::{PermissionDecision, TuiEvent};
use glob::glob;
...
impl ShellSessionState {
    ...
    async fn execute_command(
        ...
        command_chain.push(format!(
            "cd {} || {{ echo 'ERROR: Failed to change to directory: {}' >&2; exit 1; }}",
        ...
        command_chain.push(format!(
            "pwd -P >| {}",
            shell_words::quote(&cwd_file.to_string_lossy())
        ));
        ...
        let env_file = if self.advanced_persistence {
            ...
            command_chain.push(format!(
                "(set; echo '---EXPORTS---'; env) > {}",
                shell_words::quote(&env_file.to_string_lossy())
            ));
            Some(env_file)
        } else {
            None
        ...
        let output = if let Some(token) = cancellation_token {
            ...
            let output_future = async {
                ...
                stdout_result
                    .map_err(|e| Error::Process(format!("Failed to read stdout: {}", e)))?;
                stderr_result
                    .map_err(|e| Error::Process(format!("Failed to read stderr: {}", e)))?;
...
impl ToolExecutor {
    ...
    pub fn new() -> Self {
        ...
        tools.insert("EmacsCommand".to_string(), Box::new(EmacsCommandTool));
    ...
    pub async fn execute_with_context(
        ...
        let hook_context =
            HookContext::new(HookType::PreToolUse, &session_id).with_tool(name, input.clone());
                ...
                if let Some(event_tx) = &context.event_tx {
                    ...
                    if let Some(todos_array) = input["todos"].as_array() {
                        ...
                        for todo_value in todos_array {
                            if let Ok(todo) = serde_json::from_value::<crate::ai::todo_tool::Todo>(
                                todo_value.clone(),
                            ) {
                                todos.push(todo);
                            }
...
- If you read a file that exists but has empty contents you will receive a system reminder warning in place of file contents.".to_string()
    ...
    async fn execute(
        ...
        use crate::permissions::{FileOperation, PermissionBehavior, PERMISSION_CONTEXT};
        ...
        tracing::debug!(
            "DEBUG: Reading file: {}, offset: {}, limit: {:?}",
            path,
            offset,
            limit
        );
        ...
        tracing::debug!(
            "DEBUG: Checking permissions for read operation on: {}",
            path
        );
        {
            ...
            tracing::debug!(
                "DEBUG: Permission check for {}: {:?}",
                path,
                perm_result.behavior
            );
        ...
        if !file_path.exists() {
            ...
            if let Ok(cwd) = std::env::current_dir() {
                if let Some(home) = dirs::home_dir() {
                    if cwd != home {
                        error_msg
                            .push_str(&format!(" Current working directory: {}", cwd.display()));
                    }
                }
            }
                            ...
                            for entry in entries.filter_map(|e| e.ok()) {
                                ...
                                if let Some(entry_str) = entry_name.to_str() {
                                    ...
                                    if entry_stem == file_stem && entry_str != file_name {
                                        error_msg
                                            .push_str(&format!(" Did you mean {}?", entry_str));
                                        break;
                                    }
                                }
                            }
        ...
        if file_size > MAX_FILE_SIZE {
            ...
            return Err(Error::InvalidInput(format!(
                "File \"{}\" is too large: {:.0}KB (max: {:.0}KB)",
                path, size_kb, max_kb
            )));
        }
        ...
        let image_extensions: HashSet<&str> = [
            ...
            "png", "jpg", "jpeg", "gif", "webp", // Rust enhancements
        ...
        let binary_extensions: HashSet<&str> = [
            // Audio
            "mp3", "wav", "flac", "ogg", "aac", "m4a", "wma", "aiff", "opus", // Video
            "mp4", "avi", "mov", "wmv", "flv", "mkv", "webm", "m4v", "mpeg", "mpg",
            // Archives
            "zip", "rar", "tar", "gz", "bz2", "7z", "xz", "z", "tgz", "iso",
            // Executables/Libraries
            "exe", "dll", "so", "dylib", "app", "msi", "deb", "rpm", // Binary data
            "bin", "dat", "db", "sqlite", "sqlite3", // Other binary formats
            "pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "class", "pyc", "pyo", "o", "obj",
            "a", "lib", "ttf", "otf", "woff", "woff2", "eot", "cur", "icns", "psd", "ai", "eps",
            "dmg", "pkg", "apk", "ipa",
        ]
        .iter()
        .cloned()
        ...
        if is_image {
            ...
            if file_size == 0 {
                return Err(Error::InvalidInput(
                    "Empty image files cannot be processed.".to_string(),
                ));
            }
...
- Only use emojis if the user explicitly requests it. Avoid writing emojis to files unless asked.".to_string()
    ...
    async fn execute(
        ...
        tracing::debug!(
            "DEBUG: Writing file: {} ({} bytes), mode: {}, create_dirs: {}",
            path,
            content.len(),
            mode,
            create_dirs
        ...
        // Check permissions before file access
        tracing::debug!(
            "DEBUG: Checking permissions for write operation on: {}",
            path
        );
        {
            let mut ctx = PERMISSION_CONTEXT.lock().await;
        ...
        let old_content = if exists && mode == "overwrite" {
            match async_fs::read_to_string(path).await {
                Ok(content) => {
                    tracing::debug!(
                        "DEBUG: Successfully read existing content from {} ({} bytes)",
                        path,
                        content.len()
                    );
                    Some(content)
                ...
                Err(e) => {
                    tracing::debug!(
                        "DEBUG: Failed to read existing content from {}: {}",
                        path,
                        e
                    );
                    None
        ...
        if create_dirs {
            if let Some(parent) = path_obj.parent() {
                ...
                match async_fs::create_dir_all(parent).await {
                    Ok(_) => tracing::debug!(
                        "DEBUG: Successfully created parent directories for: {:?}",
                        parent
                    ),
                    Err(e) => {
                        tracing::error!(
                            "DEBUG: Failed to create parent directories for {:?}: {}",
                            parent,
                            e
                        );
        ...
        if mode == "append" {
            ...
            tracing::debug!(
                "DEBUG: Overwriting file: {} with {} bytes",
                path,
                content.len()
            );
            match async_fs::write(path, &content).await {
                Ok(_) => tracing::info!(
                    "DEBUG: File write successful: {} ({} bytes)",
                    path,
                    content.len()
                ),
                Err(e) => {
            ...
            if let Some(old) = old_content {
                ...
                if !inline_diff.is_empty() && inline_diff.len() < 500 {
                    tracing::debug!(
                        "DEBUG: Including inline diff in response ({} chars)",
                        inline_diff.len()
                    );
                    Ok(format!("{}\n\n{}", summary, inline_diff))
                } else {
                    tracing::debug!(
                        "DEBUG: Inline diff too large ({} chars), returning summary only",
                        inline_diff.len()
                    );
                    Ok(summary)
                }
    ...
    async fn execute(
        ...
        // Check permissions before directory access
        {
            ...
            match perm_result.behavior {
                PermissionBehavior::Deny | PermissionBehavior::Never => {
                    return Err(Error::PermissionDenied(format!(
                        "Permission denied to list directory: {}",
                        path.display()
                    )));
                }
                PermissionBehavior::Ask => {
        ...
        for entry in read_dir {
            ...
            let should_ignore = ignore_globs.iter().any(|pattern| {
                pattern.matches(&file_name) || pattern.matches(&entry_path.to_string_lossy())
            });
    ...
    async fn execute(
        ...
        // Check permissions before directory access
        {
            ...
            match perm_result.behavior {
                PermissionBehavior::Deny | PermissionBehavior::Never => {
                    return Err(Error::PermissionDenied(format!(
                        "Permission denied to search in: {}",
                        path
                    )));
                }
                PermissionBehavior::Ask => {
    ...
    async fn execute(
        ...
        if which("rg").is_err() {
            return Err(Error::ToolExecution(
                "ripgrep (rg) is not installed. Please install ripgrep to use the Grep tool."
                    .to_string(),
            ));
        }
        ...
        if output_mode == "files_with_matches" {
            ...
            if let Some(limit) = input["head_limit"].as_u64() {
                ...
                if lines.len() == limit as usize && result.lines().count() > limit as usize {
                    result = format!(
                        "{}\n\n[Output limited to first {} entries]",
                        lines.join("\n"),
                        limit
                    );
                } else {
...
- Use `replace_all` for replacing and renaming strings across the file. This parameter is useful if you want to rename a variable for instance.".to_string()
    ...
    async fn execute(
        ...
        // Check permissions before file access
        tracing::debug!(
            "DEBUG: Checking permissions for edit operation on: {}",
            file_path
        );
        {
            let mut ctx = PERMISSION_CONTEXT.lock().await;
        ...
        let content = match async_fs::read_to_string(file_path).await {
            Ok(content) => {
                tracing::debug!(
                    "DEBUG: Successfully read file {} ({} bytes)",
                    file_path,
                    content.len()
                );
                content
        ...
        let result = if replace_all {
            ...
            if let Some(pos) = content.find(old_string) {
                ...
                return Err(Error::InvalidInput(
                    "String not found in file. Failed to apply edit.".to_string(),
                ));
            }
        };
        ...
        if result == content {
            return Err(Error::InvalidInput(
                "Original and edited file match exactly. Failed to apply edit.".to_string(),
            ));
        }
    ...
    async fn execute(
        ...
        for (idx, edit) in edits.iter().enumerate() {
            let old_string = edit["old_string"].as_str().ok_or_else(|| {
                Error::InvalidInput(format!("Missing 'old_string' in edit {}", idx + 1))
            })?;

            let new_string = edit["new_string"].as_str().ok_or_else(|| {
                Error::InvalidInput(format!("Missing 'new_string' in edit {}", idx + 1))
            })?;
 ...
 - If you _still_ need to run `grep`, STOP. ALWAYS USE ripgrep at `rg` first, which all Claude Code users have pre-installed.
  ...
  - Try to maintain your current working directory throughout the session by using absolute paths and avoiding usage of `cd`. You may use `cd` if the User explicitly requests it.
    ...
    async fn execute(
        ...
        use crate::permissions::{PermissionBehavior, PERMISSION_CONTEXT};
        ...
        let timeout_ms = input["timeout"].as_u64().unwrap_or(120000).min(600000); // Max 10 minutes
        ...
        tracing::debug!(
            "DEBUG: Shell parameters - working_dir: {:?}, sandboxed: {}, stream: {}",
            working_dir,
            is_sandboxed,
            stream_output
        );
        ...
        if run_in_background {
            ...
            let mut cmd =
                tokio::process::Command::new(shell_executable.as_deref().unwrap_or("/bin/bash"));
            cmd.arg("-c");
            cmd.arg(command);
                ...
                Ok(mut child) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        match process.wait().await {
                            Ok(status) => {
                                if let Some(shell) =
                                    BACKGROUND_SHELLS.get_shell(&shell_id_monitor).await
                                {
        ...
        if is_sandboxed && !cfg!(target_os = "macos") {
            return Err(Error::InvalidInput(
                "Sandbox mode requested but not available on this system".to_string(),
            ));
        }
        ...
        let mut session_state = if let Some(existing_session) = sessions_guard.get_mut(shell_id) {
            ...
            let mut session_state =
                ShellSessionState::new(shell_executable, working_dir.clone(), is_sandboxed);
        ...
        let (stdout, stderr, exit_code) = session_state
            .execute_command(
                command,
                timeout_ms,
                additional_env.as_ref(),
                cancellation_token,
            )
            .await?;
        ...
        if exit_code == 0 {
            result.push_str(&format!(
                "Executed successfully in {:.2}ms\n\n",
                elapsed.as_secs_f64() * 1000.0
            ));
        } else {
            result.push_str(&format!(
                "Command FAILED with exit code {} after {:.2}ms\n\n",
                exit_code,
                elapsed.as_secs_f64() * 1000.0
            ));
        }
        ...
        if exit_code != 0 && stderr.is_empty() && stdout.is_empty() {
            result.push_str(
                "(No output captured - command may have failed to execute or been blocked)\n",
            );
        }
...
impl ToolHandler for BashOutputTool {
    ...
    fn action_description(&self, input: &serde_json::Value) -> String {
        format!(
            ...
            input["task_id"].as_str().unwrap_or("<unknown>")
        )
    }
                ...
                if let Some(ref token) = cancellation_token {
                    if token.is_cancelled() {
                        return Err(Error::Cancelled(
                            "Task output retrieval cancelled".to_string(),
                        ));
                    }
                }
            ...
            if let Ok(re) = regex::Regex::new(filter_regex) {
                if let Some(stdout) = output_json["stdout"].as_str() {
                    let filtered_lines: Vec<&str> =
                        stdout.lines().filter(|line| re.is_match(line)).collect();
...
impl ToolHandler for KillBashTool {
    ...
    fn action_description(&self, input: &serde_json::Value) -> String {
        format!(
            "Kill shell: {}",
            input["shell_id"].as_str().unwrap_or("<unknown>")
        )
    }
    ...
    async fn execute(
        ...
        if killed {
            Ok(format!(
                "Successfully killed background shell: {}",
                shell_id
            ))
        } else {
            Ok(format!(
