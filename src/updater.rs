...
pub async fn check_for_updates() -> Result<UpdateStatus> {
    ...
    Ok(UpdateStatus {
        ...
        update_channel: if is_running_from_local() {
            "local".to_string()
        } else {
            "global".to_string()
        },
...
pub async fn migrate_to_local() -> Result<()> {
    ...
    fs::create_dir_all(&local_path).context("Failed to create local installation directory")?;
