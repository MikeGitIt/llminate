...
async fn test_env_credential_provider_success() {
    ...
    assert_eq!(
        ...
        Some("20230101/us-east-1/s3/aws4_request".to_string())
    );
    assert_eq!(creds.account_id, Some("123456789012".to_string()));
    assert!(creds.source.is_some());
    assert_eq!(
        creds.source.as_ref().unwrap().get("CREDENTIALS_ENV_VARS"),
        Some(&"g".to_string())
    );
...
async fn test_env_credential_provider_missing_credentials() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("AWS_ACCESS_KEY_ID not found"));
}
...
async fn test_sigv4_canonical_query_string() {
    ...
    // Test URL encoding
    assert_eq!(
        signer.get_canonical_query_string("/path?key=value with spaces"),
...
async fn test_container_metadata_provider_no_env() {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("cannot be used unless"));
}
...
async fn test_sigv4_full_signing_flow() {
    ...
    let result = signer
        .sign("GET", "/", &mut headers, b"", &credentials)
        .await;
