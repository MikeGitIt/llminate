use anyhow::Result;
use llminate::{
    ai::todo_tool::{TodoReadTool, TodoWriteTool},
...
async fn test_todo_write_schema() -> Result<()> {
    ...
    // Verify enums
    let status_enum = item_schema["properties"]["status"]["enum"]
        .as_array()
        .unwrap();
    ...
    let priority_enum = item_schema["properties"]["priority"]["enum"]
        .as_array()
        .unwrap();
...
async fn test_todo_invalid_input() -> Result<()> {
    ...
    assert!(result
        .unwrap_err()
        .to_string()
        .contains("Missing or invalid 'todos' field"));
