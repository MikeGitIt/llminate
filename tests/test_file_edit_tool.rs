...
async fn test_file_edit_string_not_found() {
    ...
    assert!(
        result.is_err(),
        "FileEdit should fail when string not found"
    );

    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("String not found in file"),
        "Error should indicate string not found"
    );
...
async fn test_file_edit_same_strings() {
    ...
    assert!(
        result.is_err(),
        "FileEdit should fail when old_string equals new_string"
    );

    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("exactly the same"),
        "Error should indicate strings are the same"
    );
