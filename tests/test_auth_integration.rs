use anyhow::Result;
use llminate::auth::{AuthManager, AuthMethod, ClaudeAiOauth};
use std::env;
...
async fn test_macos_determine_auth_with_existing_keychain() -> Result<()> {
    ...
    match result {
        ...
        Err(e) => {
            println!(
                "No auth found in keychain (expected if not configured): {}",
                e
            );
        }
    }
...
async fn test_auth_priority_chain_env_vars() -> Result<()> {
    ...
    let result = auth_manager.determine_auth_method().await?;
    match result {
        AuthMethod::ClaudeAiOauth(oauth) => {
            assert_eq!(oauth.access_token, "oauth-token-priority-2")
        }
