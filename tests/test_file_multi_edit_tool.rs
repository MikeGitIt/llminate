...
async fn test_file_multi_edit_with_replace_all() {
    ...
    assert!(
        result.is_ok(),
        "FileMultiEdit with replace_all should succeed"
    );
...
async fn test_file_multi_edit_sequential_edits() {
    ...
    assert_eq!(
        new_content, expected_content,
        "Sequential edits should work correctly"
    );
...
async fn test_file_multi_edit_string_not_found() {
    ...
    assert!(
        result.is_ok(),
        "FileMultiEdit should succeed with partial edits"
    );
    ...
    assert_eq!(
        new_content, expected_content,
        "Only valid edit should be applied"
    );
...
async fn test_file_multi_edit_no_valid_edits() {
    ...
    assert!(
        result.is_err(),
        "FileMultiEdit should fail when no edits can be applied"
    );

    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("No edits could be applied"),
        "Error should indicate no edits applied"
    );
