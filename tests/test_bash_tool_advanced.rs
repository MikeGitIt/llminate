...
async fn test_bash_tool_persistent_session_basic_mode() {
    ...
    assert!(
        !output2.contains("hello_world"),
        "Variables should NOT persist in basic mode"
    );
...
async fn test_bash_tool_persistent_session_advanced_mode() {
    ...
    assert!(
        output2.contains("hello_world"),
        "Advanced persistence should maintain variables. Output was: {}",
        output2
    );
...
async fn test_bash_tool_working_directory() {
    ...
    assert!(
        result.is_ok(),
        "Command with working directory should succeed"
    );

    let output = result.unwrap();
    assert!(
        output.contains(temp_dir.to_str().unwrap()),
        "Should be in specified working directory"
    );
...
async fn test_bash_tool_error_handling() {
    ...
    assert!(
        result.is_ok(),
        "Tool should handle command errors gracefully"
    );

    let output = result.unwrap();
    assert!(
        output.contains("Exit code:") || output.contains("STDERR:"),
...
async fn test_bash_tool_shell_executable() {
    ...
    assert!(
        result.is_ok(),
        "Command with shell executable should succeed"
    );

    let output = result.unwrap();
    assert!(
        output.contains("bash"),
        "Should use specified shell executable"
    );
...
async fn test_bash_tool_sandbox_mode() {
    ...
    assert!(
        ...
        "Sandboxed command should execute"
    );
...
async fn test_bash_tool_sandbox_mode_unsupported() {
    ...
    assert!(
        result.is_err(),
        "Sandbox mode should fail on non-macOS systems"
    );

    let error_msg = result.unwrap_err().to_string();
    assert!(
        error_msg.contains("not available on this system"),
...
async fn test_bash_tool_multiple_sessions_with_advanced_mode() {
    ...
    assert!(
        output3.contains("session1_value"),
        "Session 1 should maintain its state. Output was: {}",
        output3
    );
    ...
    assert!(
        output4.contains("session2_value"),
        "Session 2 should maintain its state"
    );
...
async fn test_bash_tool_working_directory_persistence() {
    ...
    let output2 = result2.unwrap();
    assert!(
        output2.contains(temp_dir.to_str().unwrap()),
        "Working directory should persist in basic mode"
    );
