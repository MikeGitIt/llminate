...
use llminate::{
    ...
    ai::tools::{ToolExecutor, ToolHandler},
    config::{load_config, save_config, Config, ConfigScope},
};
use serde_json::json;
...
async fn test_task_tool_single_agent_execution() -> Result<()> {
    ...
    match result {
        Ok(output) => {
            ...
            assert!(
                output.contains("Task: Test task"),
...
async fn test_task_tool_parallel_execution() -> Result<()> {
    ...
    assert_eq!(
        loaded_config.parallel_tasks_count,
        Some(3),
        "Config should have parallel_tasks_count = 3 after saving"
    );
    println!(
        "Config verification: parallel_tasks_count = {:?}",
        loaded_config.parallel_tasks_count
    );
    ...
    match result {
        Ok(output) => {
            ...
            assert!(
                output.contains("=== Task completed with 3 parallel agents"),
                "Should show parallel completion"
            );
            ...
            assert!(
                output.contains("total tool uses"),
                "Should report total tool uses"
            );
...
async fn test_task_tool_error_handling() -> Result<()> {
    ...
    assert!(
        error_msg.contains("Missing 'prompt' field"),
...
async fn test_task_tool_slash_command_support() -> Result<()> {
    ...
    for (command, desc) in slash_commands {
        ...
        assert!(
            action.contains(desc),
            "Action description should include: {}",
            desc
        );
...
async fn test_task_tool_agent_filtering() -> Result<()> {
    ...
    assert!(
        has_task_tool,
        "Task tool should be available in main tool set"
    );
    ...
    assert!(
        !has_task_in_filtered,
        "Task tool should NOT be in filtered set"
    );
    ...
    match handler.execute(input.clone(), None).await {
        Ok(output) => {
            ...
            assert!(
                ...
                "First line should be task header but was: {}",
                lines[0]
            );
